...
#
# DESCRIPTION:
#   This script generates Figure 2: temporal dynamics of seagrass biomass (S) 
#   and stable carbon (CB) in response to global change stressors applied at 
#   different rates. It simulates system trajectories under “fast” (10 years) 
#   and “slow” (1000 years) rates along three stressor gradients 
#   (ma, pmax, temp). For each case, it estimates the time at which the system 
#   reaches the new equilibrium for both S and CB, and calculates the lag time 
#   between seagrass collapse and carbon loss. Outputs include a 6-panel figure 
#   ("figure2.png") illustrating these trajectories and transitions.
#
# DEPENDENCIES:
#   - deSolve
#   - dplyr
...
# ------------------------------------------------------------------------------
package_list <- c("deSolve", "dplyr", "nleqslv", "rootSolve", "ggplot2", "gridExtra")
...
rate_values <- c(10 * 365, 1000 * 365)
max_rate    <- max(rate_values)

pmax_0   <- 0.5 # pmax initial
pmax_f   <- 4.14 # pmax final
pmax_bif <- 3.97 # pmax final

temp_0   <- 298.85 # temp initial
temp_f   <- 308 # temp final
temp_bif <- 306.28 # temp bifurcation

ma_0     <- 0 # ma initial
ma_f     <- 0.0033 # ma final
ma_bif   <- 0.0031 # ma bifurcation
...
# ------------------------------------------------------------------------------
# Loop over parameters for transient simulations
# ------------------------------------------------------------------------------
for (par1 in c("ma", "pmax", "temp")) {
  
  for (rate in rate_values) {
    
    # Simulate until final stress value is reached, then hold final stress
    times_1 <- seq(0, rate, 1)
    start_1 <- initial_eq
    
    # Retrieve final equilibrium for the chosen parameter
    assign("final_eq", get(paste0("final_eq_", par1)))
    
    # --------------------------------------------------------------------------
    # Define time-varying forcing based on the parameter being studied
    # --------------------------------------------------------------------------
    if (par1 == "pmax") {
      
      # seagrass() increases pmax linearly from pmax_0 to pmax_f
      ...
      # seagrass2() holds pmax at pmax_f
      ...
      # seagrass() raises temp from temp_0 to temp_f
      seagrass <- function(t, state, parameters) {
        with(as.list(c(state, parameters)), {
      ...
      # seagrass2() fixes temp at temp_f
      seagrass2 <- function(t, state, parameters) {
        with(as.list(c(state, parameters)), {
      ...
      # seagrass() raises ma from ma_0 to ma_f
      seagrass <- function(t, state, parameters) {
        with(as.list(c(state, parameters)), {
    ...
    # --------------------------------------------------------------------------
    # Run Simulation: Stress Increase
    # --------------------------------------------------------------------------
    start_1_vector <- as.numeric(start_1[1, ])
    names(start_1_vector) <- names(start_1)
    
    # Phase 1: raise the stress from _0 to _f over 'rate'
    out_1 <- lsoda(
      y = start_1_vector,
      times = times_1,
      func = seagrass,
    ...
    # Phase 2: hold the final stress for additional time
    times_2 <- seq(rate, max_rate * 2, 1)
    start_2 <- out_1[nrow(out_1), 2:6]
    
    # Label the stress param from par1_0 to par1_f
    out_1 <- as.data.frame(out_1) %>%
      mutate(
        !!par1 := get(paste0(par1, "_0")) +
          (get(paste0(par1, "_f")) - get(paste0(par1, "_0"))) *
          time * 1 / rate
      )
    ...
    rm(out_1, out_2, start_1)
    
    # --------------------------------------------------------------------------
    # Check for times at which S and CB approach final equilibrium
    # --------------------------------------------------------------------------
    s_asymptotic <- data %>%
    ...
    # Keep extending time if system hasn't converged
    ...
    # Store final simulation in a data object
    assign(paste0("data_", rate, "_", par1), data)
    
    # Estimate the time at which the parameter crosses its bifurcation threshold
    # (we approximate by finding the midpoint row matching ~ par1_bif)
    bif_time <- data %>%
      filter(!!sym(par1) > 0.999 * get(paste0(par1, "_bif"))) %>%
      filter(!!sym(par1) < 1.001 * get(paste0(par1, "_bif"))) %>%
      slice(floor((n() + 1) / 2)) %>%
      select(time)
    
    bif_time <- as.numeric(bif_time)
    ...
    print(paste0(par1, "_", rate, ": lag = ", lag, " years"))
    
    rm(lag)
    
    # --------------------------------------------------------------------------
    # Build a Plot for Each Rate and Parameter
    # --------------------------------------------------------------------------
    assign(
      paste0("plot_", rate, "_", par1),
      ggplot() +
        geom_line(
          data = get(paste0("data_", rate, "_", par1)),
          aes(x = time / 365, y = S),
          color = "darkgreen",
          size = 1
        ) +
        scale_x_continuous(
          name = "time (years)",
          limits = c(0, c_asymptotic / 365)
        ) +
        scale_y_continuous(
          name = "S",
          sec.axis = sec_axis(~ . * 10, name = "CB")
        ) +
        theme_classic() +
        geom_line(
          data = get(paste0("data_", rate, "_", par1)),
          aes(x = time / 365, y = 0.1 * CB),
          color = "orange",
          size = 1
        ) +
        geom_vline(
          xintercept = s_asymptotic / 365,
          color = "black",
          linetype = "dashed",
          size = 1
        ) +
        geom_vline(
          xintercept = c_asymptotic / 365,
          color = "black",
          linetype = "dashed",
          size = 1
        ) +
        geom_vline(
          xintercept = bif_time / 365,
          color = "red",
          linetype = "dashed",
          size = 1
        )
...
# ------------------------------------------------------------------------------
plot <- grid.arrange(
  plot_3650_ma, plot_3650_pmax, plot_3650_temp,
  plot_365000_ma, plot_365000_pmax, plot_365000_temp,
  ncol = 3
)
